...
7. **Hike start coordinates are sanity-checked against their spot's GPS.** Every `start_maps_url` in `hikes.yaml` must point within 40 km of the spot it's attached to (via the hike's `visits` list and the spot's `gps` in `content.yaml`). `scripts/audit-hike-start-coords.mjs` runs as step 1 of `build-all.mjs` and fails the Vercel build if any hike-spot pair is further apart. `maps.app.goo.gl` short URLs are resolved once via `scripts/resolve-start-urls.mjs` and cached in `scripts/start-url-coords.json`; if you add a new short URL to `hikes.yaml` you MUST run the resolve script and commit the updated cache. Added 2026-05-18 after a Zermatt cable car hike (Valais, ~46.015°N, 7.74°E) shipped on the Schwarzsee spot card (Fribourg, ~46.666°N, 7.28°E) — 80 km off and instantly catchable by coord-vs-spot distance.
